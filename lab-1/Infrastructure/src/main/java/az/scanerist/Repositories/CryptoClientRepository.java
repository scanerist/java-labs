...
public class CryptoClientRepository {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                if (street != null && house != null) {
                    clientBuilder.AddAddress(new CryptoUserAddress(street, Integer.parseInt(house)));
                }
    ...
    public Client FindClientByNameAndSurname(String name, String surname) {
        return jdbcTemplate.query("SELECT * FROM clients WHERE name = ? AND surname = ?", new Object[]{name, surname}, (rs, rowNum) -> {
            ...
            clientBuilder.AddAddress(new CryptoUserAddress(rs.getString("street"), rs.getInt("address_building_number")));
